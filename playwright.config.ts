...
export default defineConfig({
  ...
  projects: [
    ...
    },
    {
      ...
      testIgnore: /.*submitQuote\.supplier\.spec\.ts|.*creditRequest\.supplier\.spec\.ts/,
      dependencies: ['supplier-setup'],
    },
    {
      name: 'partscheck-supplier-submitquote',
      use: { ...devices['Desktop Chrome'], storageState: '.auth/supplier.json' },
      testMatch: /.*submitQuote\.supplier\.spec\.ts/,
      dependencies: ['supplier-setup', 'partscheck-repairer'],
    },
    {
      name: 'partscheck-supplier-creditrequest',
      use: { ...devices['Desktop Chrome'], storageState: '.auth/supplier.json' },
      testMatch: /.*creditRequest\.supplier\.spec\.ts/,
      dependencies: ['supplier-setup', 'partscheck-repairer-partsorder'],
    },
    {
    ...
    },
    {
      ...
      testMatch: /.*\.spec\.ts/,
      testIgnore: /.*\.(repairerAdmin|supplier|supplierAdmin)\.spec\.ts|.*checkPrice\.spec\.ts|.*partsOrder\.spec\.ts|.*creditManagement\.spec\.ts/,
    ...
    },
    {
      ...
      dependencies: ['repairer-setup', 'partscheck-supplier-submitquote'],
    },
    {
    ...
    {
      name: 'partscheck-repairer-creditmanagement',
      use: { ...devices['Desktop Chrome'], storageState: '.auth/repairer.json' },
      testMatch: /.*creditManagement\.spec\.ts/,
      dependencies: ['repairer-setup', 'partscheck-supplier-creditrequest'],
    },
  ],
});
